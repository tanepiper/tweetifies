...
angular.module('Tweetifies')
  .controller('TimelineCtrl',
    ['$rootScope', '$scope', '$timeout', 'socket', '$timeline',
    function($rootScope, $scope, $timeout, socket, $timeline) {

      $scope.tweets = $timeline.getNewTweets().concat($timeline.getTweets());
      ...
      $scope.$watch('new_tweets', function(new_tweets) {
        $rootScope.$broadcast('newTweets', new_tweets);
      })
      ...
      socket.on('homeTimeline', function(timeline) {
        ...
        $timeline.setTweets(timeline);
        $scope.tweets = $timeline.getTweets();
      });

      socket.on('tweet', function(tweet) {
        console.log('tweet', tweet);
        $timeline.addTweet(tweet);
        $scope.new_tweets = $timeline.getNewTweets().length;
      });
